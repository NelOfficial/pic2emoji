...
COOLDOWN_TIME = os.environ.get("COOLDOWN_TIME")
...
def get_stats(message):
    ...
    bot.reply_to(message, f"total pics proceeded: {stat_file.read()}")
...
def CountStat():
    with open('stats.txt', 'r+') as stat_file:
        content = stat_file.read().strip()
        total_processed = int(content) if content else 0
    
        total_processed += 1
        
        stat_file.seek(0)
        
        stat_file.write(str(total_processed))
        stat_file.truncate()
